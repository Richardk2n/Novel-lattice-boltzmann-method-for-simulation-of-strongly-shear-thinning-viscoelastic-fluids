...
#include "guck.hpp"
using fluidx3d::util::geometry::guck::Guck;
#include "io.hpp"
...
void example(SimulationOptions& options, Parameters& parameters) {
	...
	const string path_files = fmt::format("RT-DC/{}/", id);
	...
	const long numberSteps = 500'000;//(long)(10000000*25/dtScale/ReScale);
	const uint vtkInterval = 5'000;//(uint)(500*5*25/dtScale/ReScale);  // every vtkInterval steps a vtk file is written
	initInfo(numberSteps);

	const double R_SI = 8e-6;
	const double R = 6.0f;
	...
	options.CY_a1.set(1.697);
	options.CY_a2.set(0.773);
	...
	// Define geometry
	Guck::setWalls(30, 225, 60, Lx, Ly, Lz);
	/*
	...
	*/
	...
	auto vtk_routine = [&]() {
		...
		//fw_polymer_conformationTensor.write<float>(fmt::format("{}CT{}", path_vtk+"polymerConformationTensor/", step_string_vtk));
	};
