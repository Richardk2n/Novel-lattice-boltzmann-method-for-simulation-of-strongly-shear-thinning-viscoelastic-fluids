...
\begin{document}
	...
	Alginate solutions are very important as bioinks or as scaffolding in the field of biofabrication. Alginate is also a common food additive. The solutions used here are \SI{4}{\percent} by weight of DuPont VIVAPHARM Alginate PH176 in PBS (essentially water)\todo{cite email from Tomasz Jüngst from 23.03.2023?}. The solution were prepared and measured by the Group of Tomasz Jüngst\todo{cite properly}. The measurements were done using a plate-plate rheometer. The first normal stress difference $N_1$ was calculated from the measured normal force using the following equation\cite{kulicke1977}.\todo{data so far does not include second correction term; needed info: was this plate plate? where is shear rate in PP rheometer? we claim $N_2$ is 0, therefore PP is fine, but we would really prefer CP}\todo{confirm diameter}
	\begin{equation}
		N_1 = \frac{2F}{\pi R^2} + \frac{3}{20}\rho\omega^2R^2
	\end{equation}
	Where $F$ is the measured normal force, $R$ is the radius of the rheometer, $\rho$ is the density of the fluid and $\omega$ is the angular velocity of the rheometer.
	
	The data for viscosity and first normal stress difference was fitted simultaneously for all 21 individual measurements in the dataset. The results can be seen in figures \ref{fig:fit_alginate_eta} and \ref{fig:fit_alginate_N}. The steps that can be seen in the plot of the first normal stress difference are due to the limited resolution of the rheometer used. It can be seen, that the data varies significantly, due to variations on preparation and/or measurement. The fit does not reproduce the shape perfectly but does always stay within the variation of the data. This is sufficient. While setting $\eta_\text{s} = \SI{1}{\milli\pascal\second}$, the parameters obtained are as follows.\todo{subject to change; different than before!!}
	\begin{align}
	...
	\todo{mostly mention $R_\eta$ for different fluids}
	\todo{do}
	...
	We need this solution to compare to literature. However, this solution does not include $\eta_\text{s}$, which is required in general. With the inclusion of the solvent viscosity (for details see appendix \ref{app:PTT}), the solution reads as follows.\todo{also wrong}
	\begin{equation}
		u\left(r\right) = \frac{Gl^2}{2^{j+1}\eta_\text{s}}\left[\ln\left(1+\frac{\eta_\text{s}}{\eta_\text{p}}e^\frac{R^2}{l^2}\right)-\ln\left(1+\frac{\eta_\text{s}}{\eta_\text{p}}e^\frac{r^2}{l^2}\right)\right]
	\end{equation}
	...
	\subsection{2D Poiseuille vs theory}\todo{rename to channel flow?}\todo{include cylindrical nozzle from conical nozzle sim}
	
	\subsection{3D Poiseuille vs theory}\todo{rename to pipe flow?}
	...
	\section{Results for non analytically solvable systems}\todo{do all of these with MC}\todo{Do we want walls?}
	...
	\section{Analytical solution for PTT}\label{app:PTT}\todo{This is wrong (to the best of my knoledge) not analytically solveable -> semi analytic solution}
	...
	\begin{align}
		\dot{\gamma} &= f\left(\frac{2\lambda}{\eta_\text{p}}\left[\partial_xp\right]^2\frac{r^2}{2^{2j}}\right)\frac{1}{\eta_\text{p}}\left(\partial_xp\frac{r}{2^j}-\eta_\text{s}\dot{\gamma}\right)
		\intertext{This cannot be integrated as easily as without $\eta_\text{s}$, but is still solvable.}
		\dot{\gamma}\left\{1+\frac{\eta_\text{s}}{\eta_\text{p}}f\left(\frac{2\lambda}{\eta_\text{p}}\left[\partial_xp\right]^2\frac{r^2}{2^{2j}}\right)\right\} &= f\left(\frac{2\lambda}{\eta_\text{p}}\left[\partial_xp\right]^2\frac{r^2}{2^{2j}}\right)\partial_xp\frac{r}{\eta_\text{p}2^j} \\
		\dot{\gamma} &= \frac{f\left(...\right)}{1+\frac{\eta_\text{s}}{\eta_\text{p}}f\left(...\right)}\partial_xp\frac{r}{\eta_\text{p}2^j}
	\end{align}
	For the exponential version of PTT the following $f$ is defined as follows.
	\begin{equation}
		f\left(\Tr\ma{\tau}\right) = \exp\left(\frac{\epsilon\lambda}{\eta_\text{p}}\Tr\ma{\tau}\right)
	\end{equation}
	The equation can be solved by integration as $\dot{\gamma} = \partial_ru$. We start with the following substitution.
	\begin{equation}
		v = \frac{\epsilon\lambda^2}{\eta_\text{p}^2}\left(\partial_xp\right)^2\frac{r^2}{2^{2j-1}}
	\end{equation}
	With this $u$ is as follows.
	\begin{align}
		u\left(r\right) &= \frac{\eta_\text{p}2^{j-2}}{\epsilon\lambda^2\partial_xp}\int\frac{e^v}{1+\frac{\eta_\text{s}}{\eta_\text{p}}e^v}dv
		\intertext{Substitute $v^\prime = 1+\frac{\eta_\text{s}}{\eta_\text{p}}e^v$}
		&= \frac{\eta_\text{p}2^{j-2}}{\epsilon\lambda^2\partial_xp}\frac{\eta_\text{p}}{\eta_\text{s}}\int\frac{1}{v^\prime}dv^\prime \\
		&= \frac{\eta_\text{p}2^{j-2}}{\epsilon\lambda^2\partial_xp}\frac{\eta_\text{p}}{\eta_\text{s}}\left(\ln v^\prime + C\right) \\
		&= \frac{\eta_\text{p}2^{j-2}}{\epsilon\lambda^2\partial_xp}\frac{\eta_\text{p}}{\eta_\text{s}}\left[\ln\left(1+\frac{\eta_\text{s}}{\eta_\text{p}}e^v\right) + C\right]
		\intertext{Using $l^2:= \frac{r^2}{v^2}$}
		&= \frac{\eta_\text{p}2^{j-2}}{\epsilon\lambda^2\partial_xp}\frac{\eta_\text{p}}{\eta_\text{s}}\left[\ln\left(1+\frac{\eta_\text{s}}{\eta_\text{p}}e^\frac{r^2}{l^2}\right) + C\right]
	\end{align}
	Using $u\left(R\right)\overset{!}{=}0$ and $G = -\partial_xp$ the following is recovered.
	\begin{equation}
		u\left(r\right) = \frac{Gl^2}{2^{j+1}\eta_\text{s}}\left[\ln\left(1+\frac{\eta_\text{s}}{\eta_\text{p}}e^\frac{R^2}{l^2}\right)-\ln\left(1+\frac{\eta_\text{s}}{\eta_\text{p}}e^\frac{r^2}{l^2}\right)\right]
	\end{equation}
	...
	\subsection{3D Poiseuille vs theory}\todo{not finished}
	Like above but 3D\n
	Worst $G$.
	\iffalse
	\begin{figure}[H]
