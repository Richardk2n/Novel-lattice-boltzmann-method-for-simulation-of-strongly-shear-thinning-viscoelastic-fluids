...
The obtained parameters are given in table~\ref{tab:PTTparameters}.
	\begin{table}[H]
		...
		\SG{ add a column with $R_ \eta $ }}\RK{would be identical to the content of the $\eta_p$ column, also $R_\eta$ would be used before introduced} \SG{ Stimmt, aber ich will es auf einen Blick sichtbar machen }
...
\SG{ wo don't actually show that $R_ \eta  $ determines stability? }\RK{we do not. We could do a few simulations with $\alpha_s = 0$ and increasing $R_\eta$ until the sim breaks if you want} \SG{ wie besprochen, bau einfach die Zitate ein, das reicht dann. }\todo{do}
...
%			Paxton\cite{paxton2017} &Nivea Creme\n Alginate& 1e5\n5e4 & <1\n<5e1 & >1e5\n>1e3 & Fig. 4b \\
	...
	\SG{ cross-check these values }\RK{Done and formatted}
	}
	\label{tab:bioinks}
\end{table}
...
\SG{ and vice versa: make sure that all papers discussed here are also cited in the introduction }\RK{gupta2015\_2 not in intro} \SG{ dann setz einfach die Zitate so rein, wie du sie fuer passend haeltst. }\todo{do}
...
The first approaches avoided an explicit modeling of a viscoelastic constitutive equation and simply modified the LB collision operator to obtain a basic viscoelastic fluid obeying Jeffery's model \cite{lallemand2003}.
...
For this, ... \SG{ more details }\todo{SRT collision operator as implemented, advection as implemented, the whole as implemented appendix that was deleted}
...
In the case of alginate, we speed up the convergence and start from a situation in which the polymers are pre-stretched \SG{ give details }\RK{initialized to the semianalytical solution} \SG{ schreib am besten eine Referenz auf die Gleichung hin, muesste ja im Appendix stehen. }\todo{do}.
...
Furthermore, in agreement with \cite{malaspinas2010}, also the L2 error is defined differently than above, namely:
	...
	\end{equation}
At this point, we mention that there is a typographic mistake in eq.~(61) of \cite{malaspinas2010} where the prefactor should be 8 instead of 4. \SG{ stimmt das so? }\RK{Ja}
...
Finally, we find that the error of our method decreases with lattice resolution to the power of \num{-2}. \RK{They see diminishing returns we don't} \SG{ explain }\RK{The topmost line starts from the first green dot and also is a power of -2 line, the second point is already a little of and the third and fourth deviate more and more. One could try to do a line with a smaller power, however that does not quite fit the data either. This is particularly evident for the red points. It looks like Malaspinas has about power of -2 for small N but the slope decreases for higher N. This can be explained through other error terms (for example their artificial viscosity) becoming more relevant as the lattice resolution caused error decreases. As we do not have those our error continues to decrease as expected}
...
This means that even for fast flows, cells flowing close to the center experience less stress than cells flowing near the walls in agreement with earlier results in Newtonian fluids \cite{Mueller.2023}.
	\begin{figure}[H]
		...
		\caption{Velocity profile (a), von Mises stress (b) and shear stress (c) for methyl cellulose in a conical nozzle as used for bioprinting. For the calculated viscosity see figure~\ref{fig:Nozzle/RT-DC_eta}.\RK{The black lines currently hide our resolution shall I remove them to be consistent with the RT-DC plots or do we prefer this?}}
		\label{fig:Nozzle}
	\end{figure}
...
\RK{There are three, proceeds to name two} \SG{ ? }
...
\end{equation}
\SG{ This equation does not work, $ \dot{ \gamma  }$ is a shear rate, it should read $ \partial u_x / \partial y = \dot{ \gamma  }$. }\RK{done, was supposed to read g(y) but could not read my own handwriting when copying... Made it a little more clear}
...
With $\Tr\ma{\tau} = \tau_{xx}$. \SG{ should be $ \tau _{ xx }$? }\RK{yes}
