...
The obtained parameters are given in table~\ref{tab:PTTparameters}.
	\begin{table}[H]
		...
		\end{tabular}
		\caption{Parameters of the PTT model obtained by fitting to the rheological measurements in figures~\ref{fig:fit_alginate} and \ref{fig:fit_mc} and corresponding viscosity ratios from eq.~(\ref{eq:R_eta}).}
...
%\SG{ give details }\RK{initialized to the semianalytical solution} \SG{ schreib am besten eine Referenz auf die Gleichung hin, muesste ja im Appendix stehen. }\RK{done}.
...
Our next example is the RT-DC geometry in which cells from a reservoir are squeezed through a narrow observation channel \cite{Toepfner.2018, Fregin:2019gx} with a square cross-section of $40 \times 40 \mu$m. %\qtyproduct{40x40}{\micro\meter}.
For this simulation we use periodic boundary conditions and a constant body force corresponding to a pressure gradient of \SI{1e7}{\pascal}. \SG{ units. Is this Pa/m or is this the pressure difference between inlet and outlet? }
...
The solutions were prepared by dissolving alginate (DuPont VIVAPHARM Alginate PH176) in ultrapure water at a ratio of 4 \% w/v. 
To ensure homogeneous dissolution, the material was stored at 37$^\circ$C for 24 h and regularly mixed to ensure complete dissolution. 
The measurements were conducted with a 25 mm plate-plate geometry at a 500 $\mu$m gap on a rheometer (Anton paar MCR 702). 
A solvent trap was used to limit evaporation. 
All measurements were done at a set temperature of 25$^\circ$C as shear rate sweep. 
...
Importantly, we show in figure~\ref{fig:wiParameterStudy}~(b) that this very good agreement is independent of the shuffling parameter $\alpha_\text{s}$.
	\begin{figure}[H]
		...
		(b) The error is negligibly small and independent of the shuffling parameter $\alpha_\text{s}$. \SG{ at which $ \dot{ \gamma  }$? }}
