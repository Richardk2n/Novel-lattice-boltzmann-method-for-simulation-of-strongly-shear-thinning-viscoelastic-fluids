...
Our derivation is to the one in Oliveira \cite{oliveira1999}. For both Poiseuille flow and shear flow, we assume that the velocity is unidirectional: 
\begin{equation}
	\vec{u} = \dot{\gamma}\left(y\right)\hat{e}_x.
\end{equation}
\SG{ This equation does not work, $ \dot{ \gamma  }$ is a shear rate, it should read $ \partial u_x / \partial y = \dot{ \gamma  }$. }
...
\end{align}
for a planar flow.
In axisymmetric flows $y$ in eqs.~(\ref{eq:PTTDerivation1}, \ref{eq:PTTDerivation2}) needs to be replaced by the radial coordinate $r$. \SG{ no factor 2? }
With $\Tr\ma{\tau} = \tau_{xy}$. \SG{ should be $ \tau _{ xx }$? }
