...
Other remedies include special decompositions of the stress tensor \cite{dzanic2022} or optimization of the solvent contribution \cite{gupta2015} \SG{ what is this? }\RK{Gupta is not specific in what he does but states to "optimize" $\eta_\text{s}$. I suspect an artificial increase but do not know}.\todo{reread gupta}
...
This motivates the use of highly shear-thinning viscoelastic fluids in biofabrication.
	...
	\end{figure}\todo{larger fonts in all figures}
...
Even at the highest concentration the zero-shear viscosity is two orders of magnitude lower than for the considered alginate solution.
	\begin{figure}[H]
		...
		\caption{Rheology data of a methyl cellulose solution from \cite{buyukurganci2023}.
				Orange lines show a fit of the PTT rheological model. \SG{ include legend into figure. Maybe give color coding only in caption? }\RK{meaning ony one legend for both plots?}\SG{ larger font in figures }}
		\label{fig:fit_mc}
	\end{figure}
...
In the fit, the solvent viscosity was fixed at $\eta_\text{s} = \SI{1}{\milli\pascal\second}$ as the viscosity of the PBS buffer. \SG{ give ref }\todo{made it up}
...
The obtained parameters are given in table~\ref{tab:PTTparameters}.
	\begin{table}[H]
		...
		\SG{ add a column with $R_ \eta $ }}\RK{would be identical to the content of the $\eta_p$ column, also would be used before introduced}
		\label{tab:PTTparameters}
	\end{table}\todo{names are fucked}

We introduce the ratio between the zero-shear and the infinite-shear (i.e.\ solvent) viscosity as\todo{nope $\eta_p$ is not zero shear viscosity!}
\begin{align}
...
\SG{ wo don't actually show that $R_ \eta  $ determines stability? }\RK{we do not. We could do a plot with $alpha_s$ decreasing until the sim breaks if you want}
...
A recent comparison in 2D found that the former in general leads to better stability \cite{dzanic2022}.\todo{check}
...
\SG{ do you advect polymer stress or polymer conformation? }\RK{A}
...
\end{align}
containing an arbitrary parameter $\alpha_\text{s}$. \SG{ is there an allowed range for $ \alpha _ \mathrm{s}$? }
...
These conclusions remain robust even when varying the pressure gradient, and thus the flow velocity, over many orders of magnitude as shown in figure~\ref{fig:pressureErr}~(a) of the appendix.\RK{This is the worst case error for MC and it is tiny "remain robust" is too weak here}
	...
	\begin{figure}[H]
		...
		\SG{ make these plots for different $ \alpha _ \mathrm{s}$, either include here or put into appendix }}\RK{Why? Is identical down to fp32 precision (see appendix C)}
		\label{fig:2Dvel}
	\end{figure}\todo{maybe try if it is this identical for different $\alpha_s$ first}
...
We use the same material parameters as above, but double the pressure gradient to \SG{ XX }. \SG{ why? } \RK{where analytical solutions exist in Poiseuille geometry the 3D solution differs from the 2D solution by a factor 2. See the velocities in our sims being the same with this scaling}
...
\begin{align}
	R_\nu = \frac{\nu_\text{s}}{\nu_\text{p}+\nu_\text{s}}
\end{align}
with the kinematic solvent and the polymer viscosity $\nu_\text{s}$ and $\nu_\text{p}$ \SG{ ... }. \SG{ comment whether these definitions are identical or different than the definitions above. }\RK{different but can be transformed into each other $\frac{1}{R_\nu}-1=R_\eta$}
Furthermore, in agreement with \cite{malaspinas2010}, also the L2 error is defined slightly differently than above, namely:\todo{slightly?}
...
Finally, we find that the error of our method decreases with lattice resolution to the power of \SG{ give number, show dashed line in plot }.\RK{They see diminishing returns we don't}
...
This means that even for fast flows, cells flowing close to the center experience less stress than cells flowing near the walls in agreement with earlier results in Newtonian fluids \cite{Mueller.2023}.
	\begin{figure}[H]
		...
		\caption{Velocity profile (a) and von Mises stress (b) for methyl cellulose in a conical nozzle as used for bioprinting. \SG{ draw nozzle contour with black lines }}\RK{looks terrible}
		\label{fig:Nozzle_v}
	\end{figure}
