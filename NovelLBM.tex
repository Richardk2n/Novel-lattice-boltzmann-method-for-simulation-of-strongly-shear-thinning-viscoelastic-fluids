...
In particular the simulation of strongly shear-thinnig fluids, where the ratio between the high-shear and low-shear viscosities is large, is often prevented by stability problems.
...
We apply this novel method to simulate two realistic cell carrier fluids, methyl cellulose and alginate solutions, of which the latter exhibits a viscosity ratio exceeding 10,000. 
...
Microfluidic technoglogies involving living cells are becoming increasingly popular. 
...
Other remedies include special decompositions of the stress tensor\cite{dzanic2022} or optimization of the solvent contribution\cite{gupta2015} \SG{ what is this? }.  
...
Our technique is based on the insight that stability issues in tradtional viscoelastic LBMs arise dure to an unbalance between the very large polymer stress and the much smaller viscous stress of the solvent. 
We overcome this issue by including („shuffling“) part of the polymer stress into the viscous part of the LBM algorithm.
...
Our solution contains 4\% alginate solvated in PBS and was measured in a commerical plate-plate rheometer (see appendix  \ref{app:experiment} for experimental details).
...
Bioinks in biofabrication are characterized in general by such high viscosities to guarantee stability of the printed construct.
...
This motivates the use of highly shear-thinning viscoelastic fluids in biofabrication.
	\begin{figure}[H]
		\centering
		\adjincludegraphics[trim=0 0 0 0, clip, scale=0.5]{assets/plots/fit_alginate_eta.eps}
		\adjincludegraphics[trim=0 0 0 0, clip, scale=0.5]{assets/plots/fit_alginate_N.eps}
...
\SG{ check that PTT is given in this form in \cite{phan-thien1977,phan-thien1978} or cite other source }
...
The material derivative is
	...
	\end{equation}	
Finally, the total fluid stress $\ma{\sigma}$ is the sum of the polymer stress $ \tau $ and a solvent stress due to the Newtonian solvent with viscosity $ \eta _ \mathrm{s}$:
	\begin{align}
...
For a simple shear flow in the $xy$-plane of a PTT fluid with $ \xi =0$, an analytical expression for $ \eta $ and $N_1$ can be derived as detailed in appendix \ref{app:semianalyticalPoiseuille}:
	\begin{equation}
		\eta\left(\dot{\gamma}\right) = \frac{\eta_\text{p}}{\exp\left[0.5W_0\left(4\epsilon \mathrm{Wi}^2\right)\right]} + \eta _\mathrm{s} \label{eq:shearEta}
	\end{equation}
	\begin{equation}
...
%			Concentration/\unit{\percent} & $\eta_\text{p}/\unit{\milli\pascal\second}$ & $\lambda/\unit{\milli\second}$ & $\epsilon$ \\
			Fluid & $\eta_\text{p}/mPas$ & $\lambda/ms$ & $\epsilon$ \\
			\hline
			alginate \num{4} & \num{48,200\pm0.4} & \num{343 \pm 0.004}. & \num{0.545 \pm 0.010}\\
			methyl c. \num{0.49} & \num{18.7\pm0.4} & \num{0.344\pm0.003} & \num{0.270\pm0.003} \\
			methyl c. \num{0.59} & \num{32.5\pm0.7} & \num{0.433\pm0.004} & \num{0.365\pm0.004} \\
...
\SG{ wo don't actually show that $R_ \eta  $ determines stability? }
...
\SG{ make sure that all papers cited in the introduction as ''3D'' are discussed here as well }
\SG{ and vice versa: make sure that all papers discussed here are also cited in the introduction }
...
Other remedies include special decompositions of the stress tensor\cite{dzanic2022} or optimization of the solvent contribution\cite{gupta2015} \SG{ rephrase and give some more details }.  
...
A recent comparison in 2D found that the former in general leads to better stability \cite{dzanic2022}.
...
Existing methods often stay at or below $R_\eta = 1$ \cite{gupta2015,onishi2006,wang2020,su2013_2,gupta2015_2}, while others report being limited below $R_\eta = 10$ \cite{malaspinas2010,ma2020,kuron2021} due to stability issues. 
...
For this, ... \SG{ do we really do stress coupling? Above it said that force coupling is better? }
\SG{ give some more details, especially since we do SRT }.
...
\end{align}
containing an arbitrary parameter $\alpha _ \mathrm{s}$. \SG{ is there an allowed range for $ \alpha _ \mathrm{s}$? }
...
For a quantitative assessment, we compute the L2 error as \SG{ give correct equn. }
...
These conclusions remain robust even when varying the pressure gradient, and thus the flow velocity, over many orders of magnitude as shown in figure~\ref{fig:pressureErr}~(a) of the appendix.
	...
	\begin{figure}[H]
		...
		The time evolution of the center-line velocity is shown in the inset \SG{ to do }.
		(b) Velocity profile for methyl cellulose.
		\SG{ make these plots for different $ \alpha _ \mathrm{s}$, either include here or put into appendix }}
		\label{fig:2Dvel}
	\end{figure}
...
We use the same material parameters as above, but double the pressure gradient to \SG{ XX }. \SG{ why? }
...
The robustness against pressure gradient changes is shown in figure~\ref{fig:pressureErr}~(b) of the appendix.
	...
	\begin{figure}[H]
		...
		(b) Velocity profile for methyl cellulose.
				\SG{ make these plots for different $ \alpha _ \mathrm{s}$, either include or put into Si }}
		\label{fig:3Dvel}
	\end{figure}
...
In figure~\ref{fig:malaspinas2010} we show the L2 error as function of lattice resolution for different Weissenberg numbers $ \mathrm{Wi}$ and \SG{ XX } $R_ \nu $.
To match with \cite{malaspinas2010}, we defined these as 
\begin{align}
	XX
\end{align}
with \SG{ ... }. \SG{ comment whether these definitions are identical or different than the definitions above. }
Furthermore, in agreement with \cite{malaspinas2010}, also the L2 error is defined slightly differently than above, namely:
	\begin{equation}\label{eq:l2error}
		E_\text{u} = \frac{1}{u_\text{max}}\sqrt{\frac{1}{N}\sum\abs{u_\text{simulation}-u_\text{theory}}^2}.
	\end{equation}
\SG{ Beware equation 61 in the paper is wrong. It should have an 8 instead of the 4. Do you use equation 61? }

	\begin{figure}[H]
		\centering
		\adjincludegraphics[trim=0 0 0 0, clip, scale=1]{assets/plots/malaspinas2010.eps}
		\caption{L2 error as defined in equation \ref{eq:l2error} from Malaspinas et al.\cite{malaspinas2010} (dots) compared to our algorithm (x) as a function of grid resolution.
...
Finally, we find that the error of our method decreases with lattice resolution to the power of \SG{ give number, show dashed line in plot }.
...
The cell is initially spherical with a radius of \SI{6}{\micro\meter} and possesses a Youngs modulus of \SI{100}{\pascal} and a Poisson ratio of \SG{ XX }.
...
\SG{ connect to the ''viscoelastic wings'' paper \cite{Zhang.2020}}
...
Its key novel ingredient is the introduction of a shuffling parameter $ \alpha _ \mathrm{s}$ which shifts a substantial part of the polymer stress into the solvent contribution by artificially increasing the solvent viscosity.
...
Importantly, the outcome of the viscostiy shuffling simulations is independent of the shuffling parameter $ \alpha _ \mathrm{s}$.
...
\SG{ make a self-contained derivation which is understandable even without reading Oliveira \\}
\SG{ use Gl.~(\ref{eq:polymerStress}) as starting point and end with Poiseuille solution, but also shear solution Gl.~(\ref{eq:shearN}, \ref{eq:shearEta}) \\}
...
The shuffling is fixed at $ \alpha _ \mathrm{s} = 1$ and the viscosity as function of shear rate is computed by dividing the total fluid stress at the center of the box by the shear rate.
...
Importantly, we show in figure~\ref{fig:wiParameterStudy}~(b) that this very good agreement is independent of the shuffling parameter $ \alpha _ \mathrm{s}$.
	\begin{figure}[H]
		\centering
		\adjincludegraphics[trim=0 0 0 0, clip, scale=0.5]{assets/plots/wiParameterStudy.eps}
		\adjincludegraphics[trim=0 0 0 0, clip, scale=0.5]{assets/plots/suffleParameterStudy.eps}
		...
		(b) The error is negligibly small and independent of the shuffling parameter $ \alpha _ \mathrm{s}$.
		\SG{ plot over shear rate instead of $ \mathrm{Wi}$. Fix units of $ \eta $. }}
...
\label{sec:supplementaryFigures}
	...
	\end{figure}
		...
		\begin{figure}[H]
		\centering
		\adjincludegraphics[trim=0 0 0 0, clip, scale=0.5]{assets/plots/2DParameterStudyErr.eps}
		\adjincludegraphics[trim=0 0 0 0, clip, scale=0.5]{assets/plots/3DParameterStudyErr.eps}
