...
Our derivation is similar to the one in Oliveira \cite{oliveira1999}. The origin is placed in the center of the channel. The $x$-axis points in flow direction. For the two-dimensional case the $y$-axis points along the velocity gradient.
We separate the derivation for two and three dimensions for mathematical clarity. They are however similar. We start with shear flow in the two-dimensional case.\todo{maybe}
...
\end{equation}
\begin{equation}
	\dot{\gamma}\left(r\right) = \frac{\partial u_x}{\partial r} = \sgn\left(y\right)\frac{\partial u_x}{\partial y} \leq 0
\end{equation}
With $r = \abs{y}$.
For pure shear flow, $\dot{\gamma}$ is a constant.

With this the constitutive equation~(\ref{eq:polymerStress}) for $\xi=0$ in steady state reduces to the following. Note, that $\ma{\tau}$ is symmetric.\RK{Am I allowed to assume this is trivial?}
\begin{align}
...
\end{align}
Eliminating $\tau_{xr}$ yields the following.
\begin{equation}
...
\begin{align}
	...
	&= \eta\left(\dot{\gamma}\right) = \frac{\eta_\text{p}}{\exp\left[0.5W_0\left(4\epsilon \mathrm{Wi}^2\right)\right]} + \eta_\text{s} %\label{eq:shearEta}
\end{align}
With $\sigma_{xr} = \sigma_{xy}\sgn\left(y\right) \leq 0$.
This gives the relevant solutions for shear flow. However, these equations also hold for Poiseuille flow.

In three dimensions we pick cylindrical coordinates as follows.
\begin{align}
	x &= x\\
	y &= r\cos\varphi\\
	z &= r\sin\varphi
\end{align}
Limiting $\varphi$ to $\varphi\in\left\{0, \pi\right\}$ reproduces the above definition of $r$ in two dimensions. Other than this expansion of the definitions, the equations from above are the same in three dimension as they are in two.
\RK{The definitions of $r$ above are entirely valid in 2D and picked in a way to make the signs consistent with 3D. In my opinion this means, that we do not need to show the calculation here. But I could if you want multiple pages of boring math. See:}
...
\end{split}
\end{equation}
Multiply. Most of the products are zero, the rest can be written using different pairs of unit vectors.
\begin{equation}
...
\end{equation}
Make other term via transpose.
\begin{equation}
	\begin{split}
		0 &= \frac{\partial u_x}{\partial r}\left[2\tau_{rx}\hat{e}_x\otimes\hat{e}_x+\tau_{rr}\left(\hat{e}_x\otimes\hat{e}_r+\hat{e}_r\otimes\hat{e}_x\right)+\tau_{r\varphi}\hat{e}_x\otimes\hat{e}_\varphi + \tau_{r\varphi}\hat{e}_\varphi\otimes\hat{e}_x\right]\\
	...
	\end{split}
\end{equation}
If $\hat{e}_i\otimes\hat{e}_j$ does not appear in this equation $\tau_{ij} = 0$.
\begin{equation}
...
\end{equation}
If $\ma{\tau}$ is symmetric $\tau_{rx} = \tau_{xr}$.
\begin{equation}
...
\end{align}
The trace works out to:
\begin{equation}
	\Tr\ma{\tau} = \tau_{xx} + \tau_{rr} + \tau_{\varphi\varphi} = \tau_{xx}
\end{equation}
TODO: $N_1$? Defined along shear axis or always yy? ends up as $\tau_{xx}$ either way

}


\subsection{Channel flow}
For Poiseuille flow, we start with the Cauchy momentum equation (neglecting gravity), which reads as follows.
\begin{equation}
	\rho\frac{\Dif\vec{u}}{\Dif t} = -\nabla p + \nabla\cdot\sigma
\end{equation}
In steady state for this geometry, once integrated along $y$, this reduces to the following.
\begin{equation}
	\sigma_{xy} = \frac{\partial p}{\partial x}y
\end{equation}
The total stress experienced by the fluid is the one described by the constitutive equation plus the solvent contribution and thus the following holds.
\begin{equation}
	\tau_{xy} = \partial_xpy - \eta_\text{s}\dot{\gamma}
\end{equation}
Dividing equation~(\ref{eq:PTTDerivation1_2D}) by equation~(\ref{eq:PTTDerivation2_2D}) leads to the following relation.
\begin{equation}
	\tau_{xx} = \frac{2\lambda}{\eta_\text{p}}\tau_{xy}^2 =  \frac{2\lambda}{\eta_\text{p}}\left(\partial_xpy - \eta_\text{s}\dot{\gamma}\right)^2
\end{equation}
Inserting this back into equation~(\ref{eq:PTTDerivation2_2D}) yields the following.
\begin{align}
	\dot{\gamma} &= \exp\left(\frac{2\epsilon\lambda^2}{\eta_\text{p}^2}\left[\partial_xpy - \eta_\text{s}\dot{\gamma}\right]^2\right)\frac{1}{\eta_\text{p}}\left(\partial_xpy-\eta_\text{s}\dot{\gamma}\right)
	\intertext{This cannot be integrated as easily as without $\eta_\text{s}$, and is only solvable semi-analytically.}
	1 &= \exp\left(\frac{2\epsilon\lambda^2}{\eta_\text{p}^2}\left[\partial_xpy - \eta_\text{s}\dot{\gamma}\right]^2\right)\frac{1}{\eta_\text{p}}\left(\partial_xp\frac{y}{\dot{\gamma}}-\eta_\text{s}\right) \\
	0 &= \frac{2\epsilon\lambda^2}{\eta_\text{p}^2}\left[\partial_xpy - \eta_\text{s}\dot{\gamma}\right]^2 - \ln\eta_\text{p} + \ln\left(\partial_xp\frac{y}{\dot{\gamma}}-\eta_\text{s}\right) \\
	0 &= 2\epsilon\lambda^2\dot{\gamma}^2\left[\partial_xp\frac{y}{\dot{\gamma}\eta_\text{p}} - \frac{\eta_\text{s}}{\eta_\text{p}}\right]^2 + \ln\left(\partial_xp\frac{y}{\dot{\gamma}\eta_\text{p}}-\frac{\eta_\text{s}}{\eta_\text{p}}\right) \\
	0 &= 2\epsilon\lambda^2\dot{\gamma}^2\left[\partial_xp\frac{y}{\dot{\gamma}\eta_\text{p}} - \frac{\eta_\text{s}}{\eta_\text{p}}\right]^2 + \ln\left(\partial_xp\frac{y}{\dot{\gamma}\eta_\text{p}}-\frac{\eta_\text{s}}{\eta_\text{p}}\right)
...
\begin{align}
	...
	\intertext{Resolving for $y$ yields the following.}
	y &= \frac{\eta_\text{s}}{\partial_xp}\dot{\gamma} + \sgn\left(\dot{\gamma}\right)\frac{\eta_\text{p}}{2\lambda\sqrt{\epsilon}\partial_xp}\sqrt{W_0\left(4\epsilon\lambda^2\dot{\gamma}^2\right)}
\end{align}
We notice, that this is the solution for a poiseuille flow with viscosity $\eta_\text{s}$ expanded by a term that applies a nonuniform scaling of the profile  along the $y$-axis towards higher absolute values. We simplify again using $c < 0$ and $d > 0$.
\begin{equation}
	y = c\dot{\gamma} - R_\eta\frac{c}{d}\sqrt{W_0\left(d^2\dot{\gamma}^2\right)}\label{eq:semiAnalyticGD_2D}
\end{equation}\todo{sign and rest of explainer at the end?}
