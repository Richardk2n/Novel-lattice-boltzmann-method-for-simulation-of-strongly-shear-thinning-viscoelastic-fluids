...
We apply this novel method to simulate two realistic cell carrier liquids, methyl cellulose and alginate solutions, of which the latter exhibits a viscosity ratio of more than 10,000. 
...
In all these applications cells are suspended in a cell carrier fluid which typically contains dissolved polymers making the fluid highly viscoelastic. 
...
Other remedies include special decompositions of the stress tensor\cite{dzanic2022} or optimization of the solvent contribution\cite{gupta2015}. 
So far, however, none of these methods has been applied to very high viscosity ratio fluids which are often used as cell carrier fluids in lab experiments or technological applications. 
...
For a simple shear flow in the $xy$-plane, an analytical expression for $ \eta $ and $N_1$ is given by \cite{ferras2019} which (with $\xi=0$) reads
	\begin{equation}
		\eta\left(\dot{\gamma}\right) = \frac{\eta_\text{p}}{\exp\left[0.5W_0\left(4\epsilon \mathrm{Wi}^2\right)\right]} + \eta _\mathrm{s} \ma{D} \label{eq:shearEta}
	\end{equation}
	\begin{equation}
...
These conclusions remain robust even when varying the pressure gradient, and thus the flow velocity, over many orders of magnitude as shown in figure~\ref{fig:2DParameterStudyErr} of the appendix.
...
Finally, we compare the quality of our algorithm by comparing to an existing study on viscoelastic LBM by Malapsinas \textit{et al.} \cite{malaspinas2010}.
...
We finally demonstrated the usability of our approach in realistic geometries such as those constricting microchannels and conical bioprinting nozzles.
...
Figure~\ref{fig:wiParameterStudy}~(a) shows that our simulations are in very good agreement with the semi-analytical solution as calculated in \SG{ XX } with a maximum error of \num{2e-11}.
Importantly, we show in figure~\ref{fig:wiParameterStudy}~(b) that this very good agreement is independent of the shuffling parameter $ \alpha _ \mathrm{s}$.
	\begin{figure}[H]
		...
		(b) The error is negligibly small independent of the shuffling parameter $ \alpha _ \mathrm{s}$.}
...
\label{sec:supplementaryFigures}
	...
	\begin{figure}[H]
		...
		All other parameters are as in figure~\ref{fig:2DAlginateU}~(b) of the main text.
