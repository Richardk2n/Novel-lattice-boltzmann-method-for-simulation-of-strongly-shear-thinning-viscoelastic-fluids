...
%			Author & Material & $\eta_0/\unit{\pascal\second}$ & $\eta_\infty/\unit{\pascal\second}$ & $R_\eta$ & extracted from \\
	...
	\subsection{Channel flow vs theory}\label{sec:channelFlow}
	...
	For the 2D case above, the walls are smooth. For the 3D case the round pipe has to be approximated leading to a staircase pattern that is not accurate. The error introduced by this depends on the lattice resolution. We use a diameter of 41 lattice points here. We use the same parameter set as above, aside from doubling the pressure gradient and also initialize the polymers in a stressed state. The resulting cross-section can be seen in figure \ref{fig:3DAlginateU}.
	
	\begin{figure}[H]
		\centering
		\adjincludegraphics[trim=0 0 0 0, clip, scale=1]{assets/plots/3DAlginateU.eps}
		\caption{Plot of the cross-section of the velocity of an alginate pipe flow. Likely not fully in equilibrium.}
		\label{fig:3DAlginateU}
	\end{figure}
	\begin{figure}[H]
		\centering
		\adjincludegraphics[trim=0 0 0 0, clip, scale=1]{assets/plots/3DAlginateErr.eps}
		\caption{Plot of error of the cross-section of the velocity of an alginate pipe flow. Likely not fully in equilibrium.}
		\label{fig:3DAlginateErr}
	\end{figure}
	As can easily be seen in figure \ref{fig:3DAlginateErr} in comparison to the 2D case, the approximation of the walls carries considerable error. The L2 error is \num{0.014}. Still the error is acceptable, particularly for this moderate lattice resolution. Further validation of our implementation (using the faster-to-simulate methyl cellulose parameters and initialization at $0$) ca be found in appendix \ref{app:validation}.
	...
	\section{Viscosity Ratio in literature}\label{app:viscosityRatio}
		The viscosity ratios used in literature are limited. The following table contains overview of the maximum ratios typically used in literature. The last column shows how the ratio was calculated from the parameters given in the publications.
		\begin{center}
